...
export function AdminBlogPostPage() {
  ...
  const { data: comments = [], refetch: refetchComments } = useQuery({
    queryKey: ['admin_blog_comments', postId],
    queryFn: async () => {
      const { data, error } = await supabase
        .from('admin_blog_comments')
        .select('*')
        .eq('post_id', postId)
  ...
  useEffect(() => {
    if (post && post.id) {
      supabase.rpc('increment_admin_blog_post_view_count', { post_id: post.id });
    }
  }, [post?.id]);
  ...
  async function handleCommentSubmit(e: React.FormEvent) {
    ...
    setSubmitting(true);
    await supabase.from('admin_blog_comments').insert({
      post_id: postId,
      user_id: user?.id,
  ...
  return (
    <div className="max-w-2xl mx-auto p-6">
      <Link to="/admin-blog" className="text-orange-600 hover:underline text-sm">&larr; Back to Admin Blog</Link>
      ...
      {canComment ? (
        ...
        <div className="text-muted-foreground mb-4">You must be signed in to comment.</div>
      )}
